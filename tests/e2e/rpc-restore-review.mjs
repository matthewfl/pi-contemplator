...
async function pendingProbeRestart() {
	...
	const server = new ModelServer(async (request, res) => {
		...
		if (request.role === "contemplator") {
			...
			if (state.probeSent) return sendSse(res, { tool: { id: "restart-no-intervention", name: "no_intervention", arguments: {} } });
			await sleep(500);
...
async function reviewerRestart() {
	...
	const server = new ModelServer(async (request, res) => {
		...
		if (request.role === "contemplator") {
			...
			if (state.reviewRequested) return sendSse(res, { tool: { id: "review-restart-no-intervention", name: "no_intervention", arguments: {} } });
...
async function reviewerBudgetExhaustion() {
	...
	const server = new ModelServer(async (request, res) => {
		...
		if (request.role === "contemplator") {
			...
			if (state.requested) return sendSse(res, { tool: { id: "budget-no-intervention", name: "no_intervention", arguments: {} } });
