...
* **Memory views.** `/om:view` shows visible memory, `/om:view full` shows the full current memory state, and `/om:view contemplator` shows the contemplator's private history and probes. Use `/om:status` for visible-vs-full drift and for the separate visible observation pool vs active observation pool.
