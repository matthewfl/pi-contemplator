...
`test:e2e` starts a local OpenAI-compatible model server and launches isolated real Pi CLI processes in RPC mode while mocking only the external model-server boundary. It drives deterministic observer → reflector → dropper → contemplator → primary-agent and contemplator → reviewer flows, including overlapping background work, probe feedback, accepted proposals, rejected reviews, durable transcripts, and delivery acknowledgements. It also queues a probe during a real three-second bash sleep and verifies that the first provider request after tool completion contains both the tool output and steer, and exercises the contemplator's `search_memories` → `recall` → `send_probe` tool sequence against persisted memory and source evidence. A separate compaction suite verifies an agent-authored `compact_context` call, a contemplator steer queued immediately before compaction, post-compaction delivery and continuation, and proactive automatic compaction with no additional user prompt.
